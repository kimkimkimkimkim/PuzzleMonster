    ...
    private BattleActionValueData GetActionValue(BattleMonsterIndex doMonsterIndex, BattleMonsterIndex beDoneMonsterIndex, SkillEffectMI skillEffect, BattleActionType actionType, string skillGuid, int skillEffectIndex)
    {
        ...
        switch (skillEffect.type)
        {
            ...
            case SkillType.Damage:
                switch (skillEffect.valueTargetType)
                {
                    ...
                    case ValueTargetType.TargetMaxHp:
                        data = new BattleActionValueData() { value = GetActionValueReferenceHp(doBattleMonster, beDoneBattleMonster, skillEffect, skillGuid, skillEffectIndex) };
                        break;
                    // ダメージを参照する系の値を取得
            ...
            case SkillType.WithoutFactorDamage:
                data = new BattleActionValueData() { value = GetActionValueWithoutFactor(doBattleMonster, beDoneBattleMonster, skillEffect, skillGuid, skillEffectIndex) };
                break;

            case SkillType.Heal:
                data = new BattleActionValueData() { value = GetHealValue(doBattleMonster, beDoneBattleMonster, skillEffect, skillGuid, skillEffectIndex) };
                break;

            case SkillType.Revive:
                data = new BattleActionValueData() { value = GetActionValueRevive(doBattleMonster, beDoneBattleMonster, skillEffect, skillGuid, skillEffectIndex) };
                break;

            case SkillType.EnergyUp:
            case SkillType.EnergyDown:
                data = new BattleActionValueData() { value = GetActionValueEnergy(doBattleMonster, beDoneBattleMonster, skillEffect, skillGuid, skillEffectIndex) };
                break;
