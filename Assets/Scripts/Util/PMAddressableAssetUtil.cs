...
public class PMAddressableAssetUtil {
    ...
    /// <summary>
    /// モンスターアイコンのスプライトアトラスを取得する
    /// </summary>
    public static IObservable<SpriteAtlas> GetMonsterIconSpriteAtlasObservable(long monsterId) {
        var address = ASSET_PATH_PREFIX + "/IconImage/MonsterSpriteAtlas/" + monsterId.ToString() + ".spriteatlas";
        return AddressableAssetController.LoadAssetAsObservable<SpriteAtlas>(address);
    }

    /// <summary>
    /// モンスターアイコン情報テキストを取得する
    /// </summary>
    public static IObservable<TextAsset> GetMonsterSpriteInfoObservable(long monsterId) {
        var address = ASSET_PATH_PREFIX + "/MonsterSpriteInfo/" + monsterId.ToString() + ".txt";
        return AddressableAssetController.LoadAssetAsObservable<TextAsset>(address);
    }
